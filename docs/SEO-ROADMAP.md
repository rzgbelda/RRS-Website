...
- **2026-09-05** — 9 new static SEO landing pages published, targeting regional/vertical/offer keyword clusters a competitive-intelligence report identified as unclaimed (the same report's separate "site is broken" claims were checked and found false — see the report's own Sections 04/05 vs. 06-08 distinction). 4 vertical pages (`supplies-for-rv-parks`, `-campgrounds`, `-short-term-rentals`, `-cleaning-companies`), 2 offer pages (`auto-reorder`, `quote-match` — describing the real Reorder Program and AI-assisted quote matching, deliberately not building pages for features that don't exist like trade credit terms), 3 regional pages (`hotel-supplies-north-carolina`, `-virginia`, `-outer-banks`). Every factual claim cross-checked word-for-word against `shipping-policy.html`/`index.html`; no invented delivery-speed or feature claims. Built on a new shared header/nav include (`partials/site-header.html` + `loadSiteHeader()` in `script.js`) so future static pages don't need the chrome copy-pasted — existing pages untouched. All 9 pages added to `tools/generate-sitemap.js` and `tools/seo-audit.js`; light-touch cross-links added from 7 existing category pages and the homepage.
...
| Day | Task | Why |
|---|---|---|
| 17 | Blog foundation: `/blog/` route, `articles` table, admin editor | Infrastructure only — same CMS pattern as Hero/About sections | ✅ Done (2026-09-02) — real `articles` table (not the single-row jsonb Hero/About use; wrong shape for many posts). `/blog` index + `/blog/post?slug=...` article pages, both server-rendered before crawler fetch by extending `api/product-meta.js` (same file that already does this for products — Vercel Hobby's 12-function cap is exhausted, so this reuses the existing file rather than adding a 13th). New "Blog" admin tab (owner + marketing) with a full editor: title, slug (auto-generated, edit-locks once hand-changed), excerpt, cover image, HTML body, SEO title/description overrides, draft/published status. Wired into `tools/generate-sitemap.js` (`sitemap-blog.xml`) and `tools/seo-audit.js`. Zero articles published yet — that's Day 18 |
| 18 | Write & publish first 3 cornerstone articles (buyer-intent, long-tail) | Real content, each linking to 2–3 category/product pages | ✅ Done (2026-09-02) — 3 articles published, each linking to 2 real product/category pages: `how-much-toilet-paper-should-a-hotel-stock`, `bath-towel-buying-guide-vacation-rentals-boutique-hotels`, `trash-liner-sizing-guide-right-can-liner-every-bin`. Shipped after a long live-debugging saga tracing a Supabase SQL Editor bug that misparsed hyphenated words inside single-quoted string literals — fixed by switching `body_html` to Postgres dollar-quoting |
| 19 | Internal linking pass + related-posts module | Internal links are free authority — this spends it | ✅ Done (2026-09-04) — the 3 Day 18 articles linked out to categories/products but nothing linked back. Added `CATEGORY_ARTICLE_MAP` (script.js) so product pages show a "Buying guide" callout when their category has a matching article; the 3 relevant category pages (towels, paper-products, trash-liners-can-liners) got a "Buying Guide" link before their FAQ section; blog article pages got a live-queried "More buying guides" related-articles module (article.js) that scales automatically as more articles publish |
| 20 | Backlink outreach list — 15–20 real targets (hospitality/facility directories, East Coast business directories now that RRS serves beyond NC) | A list to work from, not automated outreach | ✅ Done (2026-09-04) — 15 real, verified targets researched and compiled (hospitality/facility trade association directories including AAHOA, ARVC, ISSA; NC/regional chamber directories; general B2B directories with real domain authority like ThomasNet and BBB). One real hazard caught during research: the Washington County (NC) Chamber of Commerce's known domain has been hijacked and now redirects to an unrelated site — flagged rather than included as a usable link. Delivered as a formatted reference page for the marketing team to work outreach from |
| 21 | Publish 3 more articles | Keep the cadence real, not a one-time burst | ✅ Done (2026-09-05) — 4 articles published (one extra beyond the day's "3 more" scope, per request), covering the 4 categories with zero blog coverage after Day 18/19: `hotel-bed-sheet-thread-count-guide`, `guest-amenities-bulk-vs-individually-wrapped`, `nitrile-gloves-housekeeping-teams-sizing-guide`, `epa-registered-disinfectants-what-hotels-need`. Same standard as Day 18: real SKU/price/case_qty verified live before writing. All 4 categories added to `CATEGORY_ARTICLE_MAP` (script.js) and got the Day 19 "Buying Guide" backlink treatment on their category pages. `sitemap-blog.xml` regenerated (7 articles total) |
