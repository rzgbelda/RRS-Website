...
-- "relation is not exist" error in the Supabase SQL Editor twice in a
-- row, on both the alias-based and alias-free versions of the query.
-- Root cause: every '' (the SQL-standard escaped apostrophe) in
-- possessive/contraction text -- "Supply''s", "don''t" -- was being
-- silently collapsed to a single ' somewhere between this file and the
-- editor executing it, which closed each string literal early and left
-- the rest of that sentence (starting with the very next character,
-- often "<a href=...") as bare unquoted SQL tokens -- explaining the
-- specific "relation \"a\" does not exist" error: the parser was hitting
-- a stray, unquoted `a` from an `<a href=` tag. Every apostrophe in this
-- file's actual string content has been rewritten out entirely (no
-- possessives, no contractions) so there is nothing left for any tool in
-- that chain to mangle.
...
select
  ...
  '<li>Keep a 10&ndash;14 day buffer on hand &mdash; paper products store easily and do not expire, so overbuying slightly costs far less than running out.</li>' ||
...
select
  ...
  '<p>A real example: <a href="/product?item=rdu-twl-btw-24x50-eco">Economy 24x50 Bath Towels</a> from Room Ready Supply are 100% cotton, sold 60 to a case (5 dozen) at $41.99/case &mdash; sized specifically for exactly this kind of multi-unit, high-turnover buying rather than a small home order.</p>' ||
...
select
  ...
  '<p>The most common trash liner mistake is not about buying the wrong brand &mdash; it is buying the wrong <em>size</em>, either too small (constant overflow and tearing at the seams) or too large (wasted material and awkward bunching that looks unprofessional in guest-facing areas).</p>' ||
  ...
  '<h2>A real example</h2>' ||
  '<p>Room Ready Supply <a href="/product?item=rm3858h">38x58 NAPCO Black Heavy Duty Municipal Liners</a> are sized for large outdoor and municipal bins specifically, at $37.64/case &mdash; built to hold up under real collection-day handling rather than tearing the first time they get lifted.</p>' ||
