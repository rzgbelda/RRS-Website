...
-- Guarded with a not-exists check on slug so this migration is safe to
-- re-run without duplicating rows if it's ever applied twice.
...
) as v(slug, title, excerpt, body_html, status, meta_title, meta_description)
