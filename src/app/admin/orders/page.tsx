    ...
    <main className="dashboard-content">
      ...
      <div className="data-table">
        ...
        <table>
          <thead>
            <tr>
              <th>Order ID</th><th>Business</th><th>Contact</th><th>Items</th>
              <th>Total</th><th>Type</th><th>Date</th><th>Status</th><th>Actions</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((o) => (
              <tr key={o.id}>
                ...
                <td><span style={{ fontSize: "12px", color: o.type === "Reorder" ? "#1e40af" : "#555" }}>{o.type}</span></td>
                <td style={{ fontSize: "13px", color: "#888" }}>{o.date}</td>
                <td><span className={`badge ${STATUS_COLORS[o.status]}`}>{o.status}</span></td>
                <td>
                  <select value={o.status} onChange={(e) => updateStatus(o.id, e.target.value)}
                    style={{ border: "1px solid #e5e7eb", borderRadius: "4px", padding: "4px 8px", fontSize: "12px", cursor: "pointer" }}>
                    <option>New</option>
